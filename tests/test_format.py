...
def test_build_post_plan_includes_metadata_collapsible() -> None:
    ...
    assert "<details><summary>Metadata</summary>" in plan.summary_meta_collapsible
    assert "fingerprint: sha256:" in plan.summary_meta_collapsible
    assert "duration_ms: 2000" in plan.summary_meta_collapsible
    assert "token_usage:" in plan.summary_meta_collapsible
    assert "truncated: True" in plan.summary_meta_collapsible
    assert "redaction_found: True" in plan.summary_meta_collapsible
...
def test_build_summary_note_body_includes_tag_and_truncation_and_warning() -> None:
    ...
    assert "<details><summary>Truncation details</summary>" in body
    assert "original_chars: 1000" in body
